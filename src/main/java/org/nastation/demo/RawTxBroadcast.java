...
public class RawTxBroadcast {
    ...
    public static void main(String[] args) throws Exception {
        ...
        * */
        ...
        BigInteger gasFee = get_gasFee(nacInstanceId);
        if (gasFee.longValue() <= 0) {
            log.error("Gas fee error");
            return;
        }

        /* 3. Check: if (toAmount + gas) enough? */
        double gasFeeDouble = NumberUtil.bigIntToNacDouble(gasFee);
        double total = toAmount + gasFeeDouble;
        HttpResult httpResult = checkTokenAmountEnough(fromAddress, nacInstanceId, nacTokenId, total);
        if (!httpResult.getFlag()) {
            log.error("toAmount + gas not enough: "+httpResult.getMessage());
            return;
        }

        //tx height of from wallet account
        long txHeight = getAccountTxHeight(nacInstanceId, fromAddress, nacTokenId) + 1;
        ...
        Tx sendTx = TxService.newTx(
                TxType.TRANSFER,
                nacInstanceId, nacTokenId,
                fromAddress, toAddress, amountBigInt,
                gasFee,
                TxGasType.NAC.value,
                txHeight,
                TxContextService.newTransferContext(nacInstanceId),
                remark, 0, fromKey);
        ...
        boolean flag = broadcastTx(json, nacInstanceId);
